...
<script type="text/javascript">
	...
	function updatepowers(ID){
		showPage("更改权限","operator/powers?ID="+ID,600,350,fun);
	}
	function updatepass(ID){
		showPage("修改密码","operator/pass?ID="+ID,600,290,fun);
	}
	function fun(){
		location.href="operator/init";
	};
	function openwin(){
		showPage("新增管理员","operator/add",600,290,fun);
	};
	function edit(id){
		showPage("修改","operator/edit?ID="+id,600,340,fun);
	};
	function del(id){
...
<div class="col-xs-12 col-sm-12">
	<div class="row">
			<div class="col-xs-12">
							...
							<button type="button"  class="btn btn-info btn-sm radius-4" onclick="openwin();" name="addnew" ><span class="ui-icon icon-plus-sign white "></span>  增加管理员</button>
							</div>
					...
					<tbody>
						<c:forEach items="${requestScope.list}" var="r">
							<tr>
								...
								<td>${r.name}</td>
								<td>${r.tel}</td>
								<td class="hidden-480">${r.lastdate}</td>
								<td>${r.statusname}</td>
								<td class="hidden-480">${r.comments}</td>
								...
								<div class="visible-md visible-lg hidden-sm hidden-xs action-buttons no-padding-right "  >
									 ...
									 <a
										class="red" href="javascript:updatepowers(${r.ID})"> <i class="icon-pencil bigger-130"></i>修改权限 
									</a>
									<a
										class="blue" href="javascript:updatepass(${r.ID})"> <i class="icon-pencil bigger-130"></i>修改密码
									</a>
								</div>
